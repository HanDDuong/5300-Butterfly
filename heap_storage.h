 ...
 *     Modeled after slotted-page from Database Systems Concepts
 *
...
class SlottedPage : public DbBlock {
 public:
  ...
  // Big 5 - we only need the destructor, copy constructor, move constructor,
  // and overloaded assignment operator are unnecessary but we delete them
  // explicitly just to make sure we don't use them accidentally
 ...
 *
 * Heap file organization. Built on top of Berkely DB RecNo file. There is one our our
 * database blocks for each Berkely DB record in the RecNo file. In this way we are using
 * Berkely for buffer management and file management.
